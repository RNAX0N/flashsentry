...
void DeviceMonitor::scanExistingDevices()
{
    ...
    for (const DeviceInfo& info : connected) emit deviceConnected(info);
    for (const DeviceInfo& info : changed) emit deviceChanged(info);
    for (const QString& node : disconnected) emit deviceDisconnected(node);
}
